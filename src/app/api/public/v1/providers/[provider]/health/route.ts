...
export async function GET(request: Request, { params }: { params: Promise<{ provider: string }> }) {
  ...
  const decision = checkPublicReadAccess(request, "rest:providers:health");
  if (!decision.allowed) return publicReadRateLimitResponse(decision);
  const result = await executePublicServerAction(
    "providers.getHealth",
    { providerId } as ActionInput<"providers.getHealth">,
  ...
  return publicJsonResponse(createPublicEnvelope({
    dataSchema: getActionDefinition("providers.getHealth").outputSchema,
    data: result.data,
    path: `/api/public/v1/providers/${providerId}/health`,
