...
export async function GET(request: Request, { params }: { params: Promise<{ provider: string }> }) {
  ...
  const decision = checkPublicReadAccess(request, "rest:providers:models");
  if (!decision.allowed) return publicReadRateLimitResponse(decision);
  const result = await executePublicServerAction(
    "providers.listModels",
    { providerId } as ActionInput<"providers.listModels">,
  ...
  return publicJsonResponse(createPublicEnvelope({
    dataSchema: getActionDefinition("providers.listModels").outputSchema,
    data: result.data,
    path: `/api/public/v1/providers/${providerId}/models`,
