import { readOwnTrustState, type TrustStateReader } from "@/lib/access/trust-state";
...
export async function GET() {
  ...
  try {
    ...
    return errorResponse(503, "trust_state_unavailable", "Access status is temporarily unavailable.");
  }

  const result = await readOwnTrustState(reader);
  if (!result.ok) return errorResponse(result.status, result.code, result.message);
  return noStoreJson({ ok: true, access: result.value });
}
